...
export function ConfigPanel({ onStart, onStop, onUploadNpy, running }: Props) {
  ...
  return (
    <aside style={{
      ...
      <h2 style={{ fontSize: 15, fontWeight: 700, marginBottom: 10, letterSpacing: 0.2 }}>
        Configuration
      </h2>

      {/* Data source */}
      <section>
        <label style={{ color: "var(--muted)", fontSize: 12, display: "block", marginBottom: 8 }}>
          DATA SOURCE
        </label>
        <select
      ...
      <label style={{ display: "flex", alignItems: "center", gap: 8, cursor: "pointer" }}>
        ...
        />
        <span style={{ fontSize: 13, color: "var(--muted)" }}>verbose logging</span>
      </label>
